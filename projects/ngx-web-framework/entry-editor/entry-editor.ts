...
export class EntryEditor {
  ...
  // ToDo: Value change violates immutability requirement of signals. This is to be avoided by
  ...
  // - Replace all identity based for loop tracking => very expensive rerendering on array change
  updateSubEntry(subEntry: Entry) {
    this.entry.update(item => {
      ...
      if (match)
        ...
        item.subEntries?.push(subEntry); // ToDo: Remove case and add error instead. Shouldnt happen...
      return item;
    });
  }
